public class q3 {
    static void spiral_travel(int[][] arr){
       int first_row = 0;
       int last_row = arr.length;
       int first_col = 0;
       int last_col = arr[0].length;
       
       while(first_row<=last_row && first_col<=last_col){
          for(int j=first_row; j<=last_col; j++){
               System.out.println(arr[first_row][j]);  
               first_row++;
          }
          for(int i=last_col; i<=last_row; i++){
               System.out.println(arr[i][last_col]);  
               last_col--;
          }
          for(int j=last_row; j<=first_col; j--){
               System.out.println(arr[last_row][j]);  
               last_row--;
          }
          for(int i=first_col; i<=first_row; j--){
